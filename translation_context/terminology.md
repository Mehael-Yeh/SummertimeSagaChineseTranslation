...
## mar_kitchen.rpy 术语补充

| 英文表达 | 统一处理 | 说明 |
|---|---|---|
| `au jus` | 肉汁 | Maria 在披萨店后厨使用的食物式性比喻；保留餐饮背景和湿润含义 |
| `Yes, ma’am.` | 是，老板娘。 | Anon 对 Maria 的服从式回应，同时对应她在店里的经营者身份 |
...
| 英文表达 | 统一处理 | 说明 |
|---|---|---|
| `break` | 休息／歇会儿 | 本场先指工作间歇，随后成为性交邀请和事后回味的连续双关；需按整段衔接处理 |
...
| 英文表达 | 统一处理 | 说明 |
|---|---|---|
| `lookin’ for trouble` | 来找刺激／找刺激 | Maria 与 Anon 关系后期在披萨店后厨、储藏室使用的性邀约式双关，不按普通“找麻烦”处理 |
| `Wages.` | 工钱 | 披萨外送欠付报酬的菜单主题；金额变量 `$[saga.prop.pizza_boxes.owed:,]` 原样保留 |
| `Baby.`（菜单主题） | 宝宝 | 指谈论宝宝，不是亲昵称呼“宝贝” |
| `kid` / `handsome` | 小子／帅哥 | 用于标记 Maria 与 Anon 从长辈式审视到亲密暧昧的关系变化 |
